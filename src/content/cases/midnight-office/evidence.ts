...
export const evidence: Evidence[] = [
  ...
  },
  {
    ...
    title: "정전 기록",
    category: "DOCUMENT",
    summary: "사건 시간대와 가까운 시각에 기록된 짧은 전원 이상 로그.",
    detail:
      "건물 관리 시스템에는 12층 일부 구역에서 짧은 전원 이상이 기록되어 있다. 전체 정전은 아니었고, 특정 회의실과 복도 조명에 순간적인 전압 저하가 있었다. 기록은 자동 복구로 표시되어 있다.",
    location: "12층 배전반",
    unlockOrder: 2,
    unlockCondition: { requiresEvidenceIds: ["e2"] },
    importance: "IMPORTANT",
    observation: "정전은 짧았고 제한적이었다. 우연한 전기 문제인지, 의도된 조작인지는 불명확하다.",
    notebookEntry: "정전 기록 확인. 12층 일부 구역에서 짧은 전원 이상이 있었다.",
    unlockedQuestion: "정전은 우연이었을까, 누군가 이용한 것일까?",
  ...
  },
  {
    ...
    title: "멈춘 회의실 시계",
    category: "OBJECT",
    summary: "회의실 벽에 걸린 아날로그 시계가 특정 시각에서 멈춰 있다.",
    detail:
      "회의실 벽시계는 실제 현재 시각과 맞지 않는다. 시계 바늘은 특정 시각에 멈춰 있었고, 배터리 덮개는 살짝 열린 상태다. 바닥에는 떨어진 배터리나 파손 흔적은 보이지 않는다.",
    location: "회의실",
    unlockOrder: 2,
    importance: "CRITICAL",
    observation: "멈춘 시계가 단순 고장인지, 사건 시간과 관련된 단서인지는 아직 알 수 없다.",
    notebookEntry: "회의실 시계 확인. 시계가 멈춰 있으며 배터리 덮개가 열려 있다.",
    unlockedQuestion: "피해자는 정확히 언제 사망했을까?",
  },
];
