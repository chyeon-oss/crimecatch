#!/usr/bin/env python3
"""
CASE002 end-to-end playthrough (Playwright, 420px mobile viewport).
...
# CASE002 is gated behind a solved CASE001; seed that prerequisite directly.
FIXTURE = {
    "profile": {
        "name": "형사",
        "xp": 900,
        ...
        "title": "주임 형사",
        "reputation": 40,
        "solvedCaseIds": ["midnight-office"],
        "wrongAccusations": 0,
        "achievementsUnlocked": [],
    },
    "activeCaseId": None,
    "history": [
        {"caseId": "midnight-office", "solved": True, "perfect": False,
...
async def main() -> int:
    async with async_playwright() as p:
        ...
        await page.evaluate(
            "s => localStorage.setItem('casenote.progress.v1', s)", _json.dumps(FIXTURE)
        )
        ...
        log(f"reconstruction beats revealed = {beats}")
        body = await page.locator('[data-testid="reconstruction-view"]').inner_text()
        bleed = [w for w in ("자정의 사무실", "박민수", "이서연", "최지훈", "한유리") if w in body]
        ...
        if problems:
            for pr in problems:
                print(f"[e2e] FAIL: {pr}")
            return 1
        print("[e2e] PASS — CASE002 420px playthrough complete")
        return 0
